# GitHub - rohitg00/awesome-openclaw · GitHub
...
Latest commit
NmadeleiDev
Update OpenClaw Setup pricing and promo in managed hosting table (#62)
9b168eb
 · 
History
123 Commits
...
Fix mobile responsiveness on homepage (#61)
	


.gitignore
	
Add .gitignore for Vercel config
	


CONTRIBUTING.md
	
Update CONTRIBUTING.md and README.md for website + directory
...
Fix TOC spacing, add unique OG images for all blog posts
...
How to Secure OpenClaw on Hostinger
25 OpenClaw Use Cases
Oracle Cloud Free Tier Setup
...
Oracle Cloud - OpenClaw Docs
Always-On AI for $0/month
Self-Host on a Free VPS
Raspberry Pi Setup
...
npm install -g openclaw@latest
openclaw onboard --install-daemon
OpenClaw on Raspberry Pi - ajfisher
OpenClaw on Raspberry Pi - Adafruit
ESP32 Embedded (MimiClaw)
...
Bind to 127.0.0.1 only (never 0.0.0.0) — over 135,000 exposed instances found online
Use SSH tunnels or Tailscale Serve for remote access
Use auth profiles (system keychain): openclaw auth set ANTHROPIC_API_KEY
Configure firewall: ufw default deny incoming && ufw allow ssh && ufw allow 443/tcp && ufw enable
Use Docker for container isolation
Run regular audits: openclaw security audit --deep and openclaw doctor
Use Composio for managed OAuth (security guide)
...
OpenClaw + LM Studio Guide
OpenClaw + vLLM on AMD
MCP Server Integration
...
25 OpenClaw Use Cases - Hostinger

20 Genius Use Cases

33 Automations in 30 Minutes
...
CNX Software - PicoClaw runs on just 10MB of RAM
Enterprise Considerations
...
Add a project to the Ecosystem Directory via pull request to directory.html
